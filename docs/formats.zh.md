...
- **粗箭頭**位於圓外，沿 σ₁ 向內、沿 σ₃ 向外。
  **這些箭頭從來不是由張量計算而得。** DIAGRA 自己的說明把它們列在
  `SPECIAL CODES` 之下，81 與 82 是大號黑色的壓縮與張裂對，83-88 為小號與
  空心變體；選定代碼後程式即詢問
  ...
  也就是方向由人逐支手動輸入，直到輸入 `0` 結束迴圈。以 CH-01a 實測，σ₃ 那對
  與解相差在 0.3° 以內，但 σ₁ 那對朝相反方向各差 3.5°，這是有人照著 INFO1
  讀整數輸入的特徵，不是計算值。

  因此 archive 的圖版可能帶兩對、一對或完全沒有箭頭，與其張量無關。依 σ₁ 與 σ₃
  繪製、並略去傾伏超過 45° 的軸，是本重建版新增的便利功能；它重現了 90 個
  archive run 中的 85 個。未能重現的五個，是當年完全沒輸入箭頭（QS0711-1、
  0406-7A、一個回轉版 0404-04C）或只輸入一對（LL-3b、CH-01e）的 run，
  且沒有任何幾何規則能區分它們：QS0216-14 在 σ₃ 傾伏 36.5° 時畫了該對，
  而 QS0711-1 傾伏 35.4°、三軸與前者相差都在 2.3° 以內，卻什麼都沒畫。
  介面上的 **Arrows** 核取方框可關掉箭頭以符合這類圖版。
